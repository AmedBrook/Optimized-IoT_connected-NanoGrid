...
Q_max = 5000                                                    # maximal capacity stored on battery.
...
P_max = 10000                                                # max output from gen set A.
...
P_pv30_max = 2000                                                # (Irr_sol[k]*S_pv30 for k in V_steps)  Maximal power generated by Solar PV modules tilted by 30° (for maximal irradiance (W/m^2).
P_pv30_min = 0                                                # Minimal power generated by Solar PV modules tilted by 30° (at night irradiance = 0 W/m^2).
P_pv60_max = 2000                                                # (Irr_sol[k]*S_pv60 for k in V_steps)  Maximal power generated by Solar PV modules tilted by 60° (for maximal irradiance (W/m^2).
P_pv60_min = 0                                                # Minimal power generated by Solar PV modules tilted by 60° (at night irradiance = 0 W/m^2).
P_wind_max = 10000                                               # (1/2*V_wind[k]*Cp*rho*math.pi*R**2 for k in V_steps) Maximal power generated by the wind turbine (cut-out wind speed (kwh).  P = 0.5 Cp ρ π R2 V3
P_wind_min = 0	                                              # Minimal power generated by the wind turbine (cut-in wind speed (kwh).
P_hp_max = 1000.1                                                # (Q_h / 1 / (1 - T_L[k] / T_H) for k in V_steps) Maximal power transferred to Meteoria barn’s heat pumps (kwh).
P_hp_min = 	0                                                 # Minimal power transferred to Meteoria barn’s heat pumps (kwh).
P_sh_max =  5000.1	                                              # Maximal power transferred to the space heater (kwh).
P_sh_min =	0                                                 # Minimal power transferred to the space heater (kwh).
P_bio_max =	1000.2                                               # Maximal power generated by biomeiler compost heater (kwh).
P_bio_min =	0                                                 # Minimal power generated by biomeiler compost heater (kwh).
P_wh_max = 6000                                                  # Maximal power transferred to water tank heater (kwh).
P_wh_min = 0	                                                # Minimal power transferred to water tank heater (kwh).
P_stnb_max = 3000                                                # Maximal standby power to keep essential equipment on standby mode (kwh).
P_stnb_min = 3000	                                              # Minimal standby power to keep essential equipment on standby mode (kwh).     
       ...
       for j in range(0,9): 
        while i < 1 : 
            ...
            if len(data_stream_Irr) == 10 or len(data_stream_wind) == 10 or len(data_stream_Temp) == 10 : 
                ...
                FOC = LpVariable.dicts("FOC", V_steps, lowBound=0, cat=LpContinuous) # Fuel oil consumption of genset j.
